...
/**
 * Gera o corpo do e-mail em HTML para um novo refinamento.
 * @param {object} doc - O documento completo da coleção 'refinements'.
 * @returns {string} Uma string contendo o HTML do e-mail.
 */
export const createRefinementEmailHTML = (doc: PopulatedRefinement) => {
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Novo Refinamento de Tratamento</title>
    </head>
    <body style="${styles.body}">
      <div style="${styles.container}">
        <div style="${styles.header}">
          <h1 style="${styles.headerTitle}">Novo Refinamento de Tratamento</h1>
        </div>
        <div style="${styles.content}">
          ...
          <div style="${styles.fieldGroup}">
            <span style="${styles.label}">Solicitação de Origem:</span>
            <span style="${styles.value}">${doc.request?.orderId ? `#${doc.request.orderId}` : 'N/A'}</span>
          </div>
          ...
          <div style="${styles.fieldGroup}">
            ...
            <span style="${styles.value}">Verificando documentação</span>
          </div>
 ...
 * @param {PopulatedRefinement} doc - O documento populado da coleção 'refinements'.
 * @returns {string} O assunto do e-mail.
 */
export const createRefinementEmailSubject = (doc: PopulatedRefinement): string => {
  const customerName = doc.customer?.name || 'Doutor(a) não especificado(a)'
  const patientName = doc.patient || 'Paciente não especificado'

  return `Novo Refinamento de Tratamento: ${patientName} (Dr(a). ${customerName})`
}
