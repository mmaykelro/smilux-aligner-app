...
export const Refinements: CollectionConfig = {
  ...
  admin: {
    ...
    description: 'Formulários de prescrição para refinamento de tratamentos já finalizados.',
    ...
    beforeChange: [
      async ({ data, req, operation, originalDoc }) => {
        if (
          ...
          if (lastRefinement && lastRefinement.orderId) {
            data.orderId = lastRefinement.orderId + 1
          } else {
            data.orderId = 200
          }
    ...
    afterChange: [
      async ({ doc, req, previousDoc, operation }) => {
        await relalidatePaths({
          req,
          paths: ['/', '/refinamentos', `/refinamentos/${doc.slug}`],
        })
          ...
          try {
            ...
            if (!populatedDoc.customer || !populatedDoc.customer.email) {
              req.payload.logger.warn(
                `Refinamento ID ${doc.id} atualizado, mas não foi possível notificar o cliente (sem e-mail).`,
              )
              return
            }

            const emailSubject = refinementStatusUpdateEmailSubject(populatedDoc)
            const emailHtml = refinementStatusUpdateEmailHTML(populatedDoc)
            ...
            req.payload.logger.error(
              `Falha ao enviar e-mail de atualização para refinamento ID ${doc.id}: ${error}`,
            )
          }
        }
      },
  ...
  fields: [
    ...
    },
    {
      ...
      admin: {
        readOnly: true,
        position: 'sidebar',
        description: 'ID sequencial único gerado automaticamente quando o refinamento é finalizado.',
      },
    },
    {
      name: 'completionDate',
      ...
      admin: {
        readOnly: true,
        position: 'sidebar',
        description: 'Data em que o refinamento foi marcado como concluído.',
        date: {
    ...
    },
    {
      ...
      admin: {
        position: 'sidebar',
        description: 'Solicitação finalizada que está sendo refinada.',
      },
    },
    {
      name: 'patient',
