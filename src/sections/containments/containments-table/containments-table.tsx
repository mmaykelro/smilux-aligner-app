...
type ContainmentsTableProps = {
  totalPages: number
  containments: AdditionalAligners[]
}

export default function ContainmentsTable({ containments, totalPages }: ContainmentsTableProps) {
  ...
  const filteredAndSortedContainments = containments?.sort((a, b) => {
    ...
    <div className="sm:w-full w-[calc(100vw-30px)]">
      <div className="rounded-md border sm:overflow-auto overflow-x-scroll">
        <Table>
          ...
          <TableBody>
            {!filteredAndSortedContainments?.length ? (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-8 text-gray-500">
                  Nenhuma solicitação de contenção encontrada
                </TableCell>
              </TableRow>
            ) : (
              filteredAndSortedContainments.map((item) => (
                <TableRow key={item.publicId}>
                  ...
                  <TableCell>
                    <div className="flex items-center justify-end space-x-2">
                      <Tooltip>
                        <TooltipTrigger>
                          <Link href={`/contencoes/${item.publicId}`}>
                        ...
                        <TooltipContent>
                          <p>Visualizar solicitação de contenção</p>
                        </TooltipContent>
                      </Tooltip>

                      <Tooltip>
                        <TooltipTrigger>
                          <Link href={`/contencoes/editar/${item.publicId}`}>
                            <Button variant="outline" size="sm" className="h-8 w-8 p-0">
        ...
        <Pagination
          data={filteredAndSortedContainments}
          title="contenções"
          page={+pagina}
          limit={+limite}
