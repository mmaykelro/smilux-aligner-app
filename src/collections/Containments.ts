...
export const Containments: CollectionConfig = {
  slug: 'containments',
  admin: {
    ...ADMIN_REQUESTS_GROUP,
    description: 'Formulários de prescrição para novas solicitações de contenção.',
  },
  labels: {
    singular: {
      pt: 'Contenção',
          ...
          try {
            ...
            if (!populatedDoc.customer || !populatedDoc.customer.email) {
              req.payload.logger.warn(
                `Solicitação de contenção ID ${doc.id} atualizada, mas não foi possível notificar o cliente (sem e-mail).`,
              )
              return
            }

            const emailSubject = containmentStatusUpdateEmailSubject(populatedDoc)
            const emailHtml = containmentStatusUpdateEmailHTML(populatedDoc)
            ...
            req.payload.logger.error(
              `Falha ao enviar e-mail de atualização para solicitação de contenção ID ${doc.id}: ${error}`,
            )
          }
        }
      },
