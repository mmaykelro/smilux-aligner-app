...
/**
 * Gera o corpo do e-mail em HTML para uma atualização de refinamento.
 * @param {object} doc - O documento completo da coleção 'refinements'.
 * @returns {string} Uma string contendo o HTML do e-mail.
 */
export const updateRefinementEmailHTML = (doc: PopulatedRefinement) => {
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Atualização no Refinamento de Tratamento</title>
    </head>
    <body style="${styles.body}">
      <div style="${styles.container}">
        <div style="${styles.header}">
          <h1 style="${styles.headerTitle}">Atualização no Refinamento de Tratamento</h1>
        </div>
 ...
 */
export const updateRefinementEmailSubject = (doc: PopulatedRefinement): string => {
  const customerName = doc.customer?.name || 'Doutor(a) não especificado(a)'
  const patientName = doc.patient || 'Paciente não especificado'

  return `Atualização no Refinamento de Tratamento: ${patientName} (Dr(a). ${customerName})`
}
