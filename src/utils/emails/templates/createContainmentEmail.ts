...
type ContainmentWithCustomer = Containment & { customer: Customer }

/**
 * Cria um assunto (subject) dinâmico e tipado para o e-mail de notificação.
 * @param {Containment} doc - O documento populado da coleção 'requests'.
 * @returns {string} O assunto do e-mail.
 */
export const createContainmentEmailSubject = (doc: ContainmentWithCustomer): string => {
  ...
  return `Nova Solicitação de Contenção: ${patientName} (Dr(a). ${customerName})`
}

export const createContainmentEmailHTML = (doc: Containment) => {
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Nova Solicitação de Contenção</title>
    </head>
    <body style="${styles.body}">
      <div style="${styles.container}">
        <div style="${styles.header}">
          <h1 style="${styles.headerTitle}">Nova Solicitação de Contenção</h1>
        </div>
        <div style="${styles.content}">
          ...
          <div style="${styles.fieldGroup}">
            ...
            <span style="${styles.value}">Pedido Feito</span>
          </div>
        </div>
