...
type Request = Containment & { customer: Customer }

/**
 * Cria um assunto (subject) dinâmico e tipado para o e-mail de notificação.
 * @param {Request} doc - O documento populado da coleção 'requests'.
 * @returns {string} O assunto do e-mail.
 */
export const updateContainmentEmailSubject = (doc: Request): string => {
  ...
  return `Atualização na Solicitação de Contenção: ${patientName} (Dr(a). ${customerName})`
}

export const updateContainmentEmailHTML = (doc: Containment) => {
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Atualização na Solicitação de Contenção</title>
    </head>
    <body style="${styles.body}">
      <div style="${styles.container}">
        <div style="${styles.header}">
          <h1 style="${styles.headerTitle}">Atualização na Solicitação de Contenção</h1>
        </div>
