import { Containment } from '@/payload-types' // Use a interface que criamos antes
import { formatOrderId } from '@/utils/text'

/**
 * Cria um assunto dinâmico para o e-mail de atualização de status para o cliente.
 * @param {Containment} doc - O documento populado da solicitação.
 * @returns {string} O assunto do e-mail.
 */
export const containmentStatusUpdateEmailSubject = (doc: Containment): string => {
  const patientName = doc.patient || '-'
  const orderId = doc.orderId ? ` ${formatOrderId(doc.orderId, 'CC')}` : ''
  ...
  if (doc.status === 'in_progress') {
    return `Em Andamento: Pedido de Contenção para o paciente ${patientName}`
  }
 ...
 * @param {Containment} doc - O documento completo da solicitação.
 ...
 */
export const containmentStatusUpdateEmailHTML = (doc: Containment): string => {
  // --- Mapeamentos para texto legível ---Containment
  ...
  const getUpdateMessage = (): string => {
    ...
    if (doc.status === 'completed') {
      return `Parabéns! O pedido de contenção para o paciente ${patientName} foi concluído. Agora, o próximo passo é o envio. Caso não tenha realizado o pagamento verifique os links em seu painel.`
    }

    if (doc.status === 'in_progress') {
      return `Nossa equipe iniciou o planejamento do pedido de contenção para o paciente ${patientName}. Em breve informaremos os próximos passos.`
    }
  ...
  return `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Atualização da Solicitação de Contenção</title>
    </head>
    <body style="${styles.body}">
      <div style="${styles.container}">
        <div style="${styles.header}">
          <h1 style="${styles.headerTitle}">Atualização da sua Solicitação de Contenção</h1>
        </div>
          ...
          <table style="${styles.summaryTable}">
            <tbody>
              ...
              <tr>
                ...
                <td style="${styles.summaryTdValue}">${doc.orderId ? formatOrderId(doc.orderId, 'CC') : 'Aguardando Conclusão'}</td>
              </tr>
              <tr>
